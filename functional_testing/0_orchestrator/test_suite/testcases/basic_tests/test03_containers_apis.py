...
class TestcontaineridAPI(TestcasesBase):
    ...
    def test002_create_containers(self):
        ...
        **Test Scenario:**
        ...
        response = self.containers_api.post_containers(nodeid=self.nodeid, data=self.container_body)
        self.assertEqual(response.status_code, 201)

        self.lg.info('Make sure it created with required values, should succeed.')
        self.assertEqual(response.headers['Location'], "/nodes/%s/containers/%s" % (self.nodeid, self.container_name))
        response = self.containers_api.get_containers_containerid(self.nodeid, self.container_name)
        self.assertEqual(response.status_code, 200)
        ...
        response = self.containers_api.delete_containers_containerid(self.nodeid, self.container_name)
        ...
        self.assertFalse(self.core0_client.client.container.find(self.container_name),
    ...
    def test003_get_container_details(self):
        ...
        **Test Scenario:**
        ...
        container_name = self.create_contaienr(self.nodeid)
        self.assertTrue(container_name)

        self.lg.info('Send get nodes/{nodeid}/containers/containerid api request.')
        response = self.containers_api.get_containers_containerid(self.nodeid, container_name)
        self.assertEqual(response.status_code, 200)
        data = response.json()

        self.lg.info(' Compare results with golden value.')
        golden_value = self.core0_client.get_container_info(container_name)
    ...
    def test004_stop_and_start_container(self):
        ...
        **Test Scenario:**
        ...
        self.assertEqual(response.status_code, 201)
        self.createdcontainer.append({"node": self.nodeid, "container": self.container_name})

        self.lg.info('post:/node/{nodeid}/containers/containerid/stop.')
        response = self.containers_api.post_containers_containerid_stop(self.nodeid, self.container_name)
        self.assertEqual(response.status_code, 204)

        self.lg.info('Check that container stoped.')
        self.assertTrue(self.core0_client.wait_on_container_update(self.container_name, 60, True))

        self.lg.info('post:/node/{nodeid}/containers/containerid/start.')
        response = self.containers_api.post_containers_containerid_start(self.nodeid, self.container_name)
        self.assertEqual(response.status_code, 201)

        self.lg.info('Check that container running.')
        self.assertTrue(self.core0_client.wait_on_container_update(self.container_name, 60, False))
    ...
    def test005_get_running_jobs(self):
        ...
        **Test Scenario:**
        ...
        response = self.containers_api.get_containers_containerid_jobs(self.nodeid, container_name)
        self.assertEqual(response.status_code, 200)

        self.lg.info('Compare results with golden value.')
        running_jobs_list = response.json()
        golden_values = self.core0_client.get_container_job_list(container_name)
    ...
    def test006_post_new_job_in_container(self):
        ...
        **Test Scenario:**
        ...
        response = self.containers_api.post_containers_containerid_jobs(self.nodeid, container_name,
                                                                        self.job_body)
        self.assertEqual(response.status_code, 202)
        job_id = response.headers['Location'].split('/')[6]
        self.assertTrue(self.core0_client.wait_on_container_job_update(container_name, job_id, 100, False))

        self.lg.info('Check that you can get created job.')
        time.sleep(7)
        response = self.containers_api.get_containers_containerid_jobs_jobid(self.nodeid, container_name, job_id)
        self.assertEqual(response.status_code, 200)
    ...
    def test007_kill_all_running_jobs(self):
        ...
        **Test Scenario:**
        ...
        for i in range(0, 3):
            ...
            self.assertEqual(response.status_code, 202)
            job_id = response.headers['Location'].split('/')[6]
            self.assertTrue(self.core0_client.wait_on_container_job_update(container_name, job_id, 15, False))

        self.lg.info('Send delete nodes/{nodeid}/containers/containerid/jobs api request.')
        response = self.containers_api.delete_containers_containerid_jobs(self.nodeid, container_name)
        self.assertEqual(response.status_code, 204)
        time.sleep(5)
        ...
        self.assertEqual(len(self.core0_client.get_container_job_list(container_name)), 1)
    ...
    def test008_get_job_in_container_details(self):
        ...
        **Test Scenario:**
        ...
        self.lg.info('Compare results with golden value.')
        container_id = int(list(self.core0_client.client.container.find(container_name).keys())[0])
        ...
        response = self.containers_api.delete_containers_containerid_jobs_jobid(self.nodeid, container_name, job_id)
        self.assertEqual(response.status_code, 204)
        self.assertTrue(self.core0_client.wait_on_container_job_update(container_name, job_id, 100, True))
    ...
    def test009_post_signal_job_in_container_details(self):
        ...
        **Test Scenario:**
        ...
        response = self.containers_api.post_containers_containerid_jobs_jobid(self.nodeid, container_name,
                                                                              job_id, body)
        self.assertEqual(response.status_code, 204)
    ...
    def test010_kill_specific_job(self):
        ...
        **Test Scenario:**
        ...
        self.assertEqual(response.status_code, 202)
        job_id = response.headers['Location'].split('/')[6]
        self.assertTrue(self.core0_client.wait_on_container_job_update(container_name,
                                                                       job_id, 60, False))
    ...
    def test011_post_ping_to_container(self):
        ...
        **Test Scenario:**
        ...
        response = self.containers_api.post_containers_containerid_ping(self.nodeid, container_name)
        self.assertEqual(response.status_code, 200)
    ...
    def test012_get_state_of_container(self):
        ...
        **Test Scenario:**
        ...
        response = self.containers_api.get_containers_containerid_state(self.nodeid, container_name)
        self.assertEqual(response.status_code, 200)
    ...
    def test013_get_info_of_container_os(self):
        ...
        **Test Scenario:**
        ...
        response = self.containers_api.get_containers_containerid_info(self.nodeid, container_name)
        self.assertEqual(response.status_code, 200)
    ...
    def test014_get_running_processes_in_container(self):
        ...
        **Test Scenario:**
        ...
        self.lg.info('Send post  nodes/{nodeid}/containers/containerid/state api request.')
        response = self.containers_api.get_containers_containerid_processes(self.nodeid, container_name)
        self.assertEqual(response.status_code, 200)
    ...
    def test015_get_process_details_in_container(self):
        ...
        **Test Scenario:**
        ...
        response = self.containers_api.get_containers_containerid_processes_processid(self.nodeid, container_name,
                                                                                      str(process_id))
        self.assertEqual(response.status_code, 200)
    ...
    def test016_delete_process_in_container(self):
        ...
        **Test Scenario:**
        ...
        response = self.containers_api.delete_containers_containerid_processes_processid(self.nodeid, container_name,
                                                                                         str(process_id))
        self.assertEqual(response.status_code, 204)
        ...
        self.lg.info(' Compare results with golden value.')
        container = self.core0_client.client.container.find(container_name)
    ...
    def test017_post_signal_to_process_in_container(self):
        ...
        **Test Scenario:**
        ...
        response = self.containers_api.post_containers_containerid_processes_processid(self.nodeid, container_name,
                                                                                       str(process_id), body)
        self.assertEqual(response.status_code, 204)
    ...
    def test018_upload_file_to_container(self):
        ...
        **Test Scenario:**
        ...
        container = self.core0_client.client.container.find(self.container_name)
        self.assertTrue(container)
        container_id = int(list(container.keys())[0])
        container = self.core0_client.client.container.client(container_id)
